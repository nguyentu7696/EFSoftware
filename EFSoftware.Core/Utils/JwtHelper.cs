...
namespace EFSoftware.Core.Utils
{
    ...
    {
        public static string Generate<T>(T data, int expireInSeconds, string issuer)
        {
            ...
            {
                var jwtData = userModel.Clone();
                
                jwtData.Permissions = string.Join(",", jwtData.ListPermission.Select(x => x.ToString()).ToList());
                
                jwtData.ListPermission = null;
                
                claims = CoreHelper.GetClaimsIdentity(jwtData);
            }
            else
            {
                claims = CoreHelper.GetClaimsIdentity(data);
            }
