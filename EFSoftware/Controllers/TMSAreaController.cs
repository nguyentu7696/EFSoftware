...
{
    public class TMSAreaController : BaseController
    {
        #region Constructor

        private ITMSAreaService _tmsAreaService;

        public TMSAreaController(ITMSAreaService tmsAreaService)
        {
            _tmsAreaService = tmsAreaService;
        }
        ...
        public IActionResult GetAll()
        {
            try
            {
                var data = _tmsAreaService.GetAll();
        ...
        public IActionResult GetAllPaging(int page, int pageSize)
        {
            try
            {
                var data = _tmsAreaService.GetAllPaging(page, pageSize);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetAllPagingWithZoneLocation(int page, int pageSize, int zonelocationid, string filterByName = "")
        {
            try
            {
                var data = _tmsAreaService.GetAllPagingWithZoneLocation(page, pageSize, zonelocationid, filterByName);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetAllWithZoneLocation(int zonelocationid, string filterByName= "")
        {
            try
            {
                var data = _tmsAreaService.GetAllWithZoneLocation(zonelocationid, filterByName);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetById(int id)
        {
            ...
            else
            {
                try
                {
                    var data = _tmsAreaService.GetById(id);
                    return new OkObjectResult(new GenericResult(true, data));
                }
                catch (Exception ex)
                {
        ...
        [HttpPost]
        [Route("Add")]
        public IActionResult Add([FromBody]TMSAreaCreateViewModel Vm)
        {
        ...
        [HttpPut]
        [Route("Update")]
        public IActionResult Update([FromBody]TMSAreaViewModel Vm)
        {
            if (!ModelState.IsValid)
            {
                var allErrors = ModelState.Values.SelectMany(v => v.Errors);
