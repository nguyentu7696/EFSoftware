...
namespace EFSoftware.Controllers
{
    public class DepartmentController : BaseController
    {
        ...
        public IActionResult GetAllPaging(int page, int pageSize)
        {
            try
            {
                var data = _departmentService.GetAllPaging(page, pageSize);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetAllPagingWithCompany(int page, int pageSize, int companyid, string filterByName = "")
        {
            try
            {
                var data = _departmentService.GetAllPagingWithCompany(page, pageSize, companyid, filterByName);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetAllWithCompany(int companyid, string filterByName = "")
        {
            try
            {
                var data = _departmentService.GetAllWithCompany(companyid, filterByName);
                return new OkObjectResult(new GenericResult(true, data));
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        [Route("Update")]
        public IActionResult Update([FromBody]DepartmentViewModel Vm)
        {
            if (!ModelState.IsValid)
            {
                var allErrors = ModelState.Values.SelectMany(v => v.Errors);
        ...
        public IActionResult Delete(int id)
        {
            ...
            else
            {
                try
                {
                    string result = _departmentService.Delete(id);
